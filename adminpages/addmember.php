<?php
	...
	if(!empty($_REQUEST['action']) && $_REQUEST['action'] == 'add_member')
	{
		...
		if(empty($user) && empty($user->ID))
		{
			...
			$pmpro_required_user_fields = apply_filters('pmpro_add_member_required_user_fields', array('user_login'=>$user_login, 'user_email'=>$user_email));
			...
			if (!empty($oldusername)) {
				pmpro_setMessage(__("That username is already taken. Please try another.", "pmpro-addmember"), "pmpro_error");
				$pmpro_error_fields[] = "username";
			}
			if (!empty($oldemail)) {
				pmpro_setMessage(__("That email address is already taken. Please try another.", "pmpro-addmember"), "pmpro_error");
		...
		if(!$user_id)
		{
			pmpro_setMessage( __( "Error creating user.", "pmpro-addmember" ), "pmpro_error");
		}
		else
		{
			//other user meta
			...
			{
				global $pmpro_msg;
				$pmpro_msg = __( "The user account has been created, but there were other errors setting up membership: ", "pmpro-addmember" ) . $pmpro_msg;
			}
		}
	}
?>
	...
	<input name="saveid" type="hidden" value="<?php echo isset( $edit ) ? $edit : null; ?>" />
		...
		<tbody>
			<?php if(!empty($user_id)) { ?>
				<tr class="user">
					<th scope="row" valign="top"><label for="user_id"><?php _e('User', 'pmpro-addmember');?>:</label></th>
					<td>
			...
			<tr class="user-info" <?php if(!empty($user_id)) { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top"><label for="user_login"><?php _e('Username', 'pmpro-addmember');?>:</label></th>
				<td>
			...
			<tr class="user-info" <?php if(!empty($user_id)) { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top"><label for="user_email"><?php _e('Email', 'pmpro-addmember');?>:</label></th>
				<td>
			...
			<tr class="user-info" <?php if(!empty($user_id)) { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top"><label for="first_name"><?php _e('First Name', 'pmpro-addmember');?>:</label></th>
				<td>
			...
			<tr class="user-info" <?php if(!empty($user_id)) { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top"><label for="last_name"><?php _e('Last Name', 'pmpro-addmember');?>:</label></th>
				<td>
			...
			<tr <?php if(!empty($user_id)) { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top"><label for="user_pass"><?php _e('Password', 'pmpro-addmember');?>:</label></th>
				<td>
			...
			<tr class="user-info" <?php if(!empty($user_id)) { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top"><label for="user_notes"><?php _e('User Notes', 'pmpro-addmember');?>:</label></th>
				<td>
			...
			<tr class="user-info" <?php if(!empty($user_id)) { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top"><label for="role"><?php _e('Role', 'pmpro-addmember');?>:</label></th>
				<td>
					<?php if(!empty($user_id)) { ?>
						<?php
			...
			<tr>
				<th scope="row" valign="top"><label for="membership_level"><?php _e('Membership Level', 'pmpro-addmember');?>:</label></th>
				<td>					
			...
			<tr>
				<th scope="row" valign="top"><label for="expires_date"><?php _e('Expiration', 'pmpro-addmember');?>:</label></th>
				<td>					
					<?php
					...
					<span id="expires_date" <?php if(!$end_date) { ?>style="display: none;"<?php } ?>>
						...
						<select name="expires_month">
							<?php																
								...
								{
								?>
								<option value="<?php echo $i?>" <?php if($i == $expires_month) { ?>selected="selected"<?php } ?>><?php echo date("M", strtotime($i . "/1/" . $current_year, current_time("timestamp")))?></option>
								<?php
								}
							?>
						</select>
			...
			<tr>
				<th scope="row" valign="top"><label for="payment"><?php _e('Payment', 'pmpro-addmember');?>:</label></th>
				<td>
			...
			<tr class="payment payment-check payment-gateway payment-credit">
				<th scope="row" valign="top"><label for="total"><?php _e('Order Total', 'pmpro-addmember');?>:</label></th>
				<td>
					<?php
			...
			<tr>
				<th scope="row" valign="top"><label for="order_notes"><?php _e('Order Notes', 'pmpro-addmember');?>:</label></th>
				<td>
